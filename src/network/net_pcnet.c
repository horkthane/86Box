...
typedef struct {
	...
	volatile int rint_irq, tint_irq;
} nic_t;
...
pcnetUpdateIrq(nic_t *dev)
{
	...
	pcnetlog(2, "%s: set irq isr=%02x, temp isr=%02x\n", dev->name, !!isr, !!dev->iISR);
	...
	if (!(dev->aCSR[3] & 0x0400) && (dev->aCSR[0] & 0x0400)) {
		pclog("%s: RINT IRQ = %02x\n", dev->name, !!isr);
		network_set_wait(1);
	} else if (!(dev->aCSR[3] & 0x0200) && (dev->aCSR[0] & 0x0200)) {
		pclog("%s: TINT IRQ = %02x\n", dev->name, !!isr);
		network_set_wait(1);		
	} else {
		network_set_wait(0);
	}
...
pcnetStart(nic_t *dev)
{
    ...
    pcnetPollTimer(dev);      /* start timer if it was stopped */
}
...
pcnetReceiveNoSync(void *priv, uint8_t *buf, int size)
{
    ...
    pclog("%s: pcnetReceiveNoSync: RX %x:%x:%x:%x:%x:%x > %x:%x:%x:%x:%x:%x len %d\n", dev->name,
    ...
    if (CSR_PROM(dev)
        ...
        if (HOST_IS_OWNER(CSR_CRST(dev))) {
            ...
            dev->aCSR[0] |= 0x0400;
            pclog("%s: RINT set, RCVRC=%d CRDA=%#010x\n", dev->name,
...
pcnetAsyncTransmit(nic_t *dev)
{
    ...
    do {
        ...
        if (    !CSR_TOKINTD(dev)      /* Transmit OK Interrupt Disable, no infl. on errors. */
			...
			dev->aCSR[0] |= 0x0200;    /* set TINT */
			pclog("%s: pcnetAsyncTransmit: TINT set\n", dev->name);
			/* Don't allow the guest to clear TINT before reading it */
			pcnetUpdateIrq(dev);
        }
        if (--cMax == 0)
...
{
	uint16_t inea = CSR_INEA(dev);
	
    pclog("%s: pcnet_csr_writew: rap=%d val=%#06x\n", dev->name, rap, val);
    switch (rap) {
    case 0:
		...
		if (inea && !(val & 0x40)) {
			pclog("%s: pcnet_csr_writew(): IRQ disable\n", dev->name);
			pcnet_do_irq(dev, 0);
		}
        if (!CSR_STOP(dev) && (val & 4)) {
			pclog("%s: pcnet_csr_writew(): Stop\n", dev->name);
            pcnetStop(dev);
        }
        if (!CSR_INIT(dev) && (val & 1)) {
			pclog("%s: pcnet_csr_writew(): Start\n", dev->name);
            pcnetInit(dev);
        }
        ...
        if (CSR_TDMD(dev)) {
			pclog("%s: pcnet_csr_writew(): Transmit\n", dev->name);
			pcnetAsyncTransmit(dev);
        }
		return;
	case 2: /* IADRH */
...
pcnet_csr_readw(nic_t *dev, uint16_t rap)
{
    ...
    switch (rap) {
    case 0:
		pclog("%s: pcnet_csr_readw(): update interrupt = %02x\n", dev->name, CSR_INEA(dev));
		pcnetUpdateIrq(dev);
        val = dev->aCSR[0];
...
pcnet_word_write(nic_t *dev, uint32_t addr, uint16_t val)
{
    ...
    if (!BCR_DWIO(dev)) {
        ...
        case 0x00: /* RDP */
			pcnetPollTimer(dev);
			pclog("%s: pcnet_word_write(): going to pcnet_csr_writew()\n", dev->name);
...
{ 
    ...
    if (!BCR_DWIO(dev)) {
		switch (addr & 0x0f) {
		case 0x00: /* RDP */
			pclog("%s: pcnet_word_read(): RDP: dpoll = %04x, rap = %d, update interrupt = %02x\n", dev->name, !CSR_DPOLL(dev), dev->u32RAP, CSR_INEA(dev));
			if (!CSR_DPOLL(dev))
				pcnetPollTimer(dev);
